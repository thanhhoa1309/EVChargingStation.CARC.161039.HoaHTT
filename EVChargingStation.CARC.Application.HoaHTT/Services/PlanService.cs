...
namespace EVChargingStation.CARC.Application.HoaHTT.Services
{
    public class PlanService : IPlanService
    {
        ...
        public PlanService(ILoggerService loggerService, IUnitOfWork unitOfWork)
        {
        ...
        public async Task<PlanResponceDTOs> CreatePlanAsync(PlanRequestDTOs planRequestDto)
        {
            ...
            try
            {
                ...
                if (planRequestDto.MaxDailyKwh < 0)
                {
        ...
        public async Task<PlanResponceDTOs> GetPlanByIdAsync(Guid HoaHTTID)
        {
            try
            {
                _loggerService.Info($"[GetPlanByIdAsync] Fetching plan details for PlanId: {HoaHTTID}");
                ...
                _loggerService.Success($"[GetPlanByIdAsync] Successfully retrieved details for PlanId: {HoaHTTID}");
                return responseDto;
            }
            ...
            {
                _loggerService.Error($"[GetPlanByIdAsync] Error fetching plan details for PlanId: {HoaHTTID}. Exception: {ex.Message}");
        ...
        public async Task<PlanUpdateDTOs> UpdatePlanAsync(Guid HoaHTTID, PlanUpdateDTOs planUpdateDto)
        {
            try
            {
                _loggerService.Info($"[UpdatePlan] Attempting to update plan info for PlanId: {HoaHTTID}");

                var plan = await _unitOfWork.Plan.GetByIdAsync(HoaHTTID);
                if (plan == null || plan.IsDeleted)
                {
                    _loggerService.Warn($"[UpdatePlan] Plan with ID {HoaHTTID} not found.");
                    throw new KeyNotFoundException($"Plan with ID {HoaHTTID} not found.");
                }
                ...
                if (!isUpdated)
                {
                    _loggerService.Warn($"[UpdatePlan] No changes detected for PlanId: {HoaHTTID}");
                ...
                _loggerService.Success($"[UpdatePlan] Plan info updated successfully for PlanId: {HoaHTTID}");
            ...
            {
                _loggerService.Error($"[UpdatePlan] Error updating plan info for PlanId: {HoaHTTID}. Exception: {ex.Message}");
                throw;
            }
        }
