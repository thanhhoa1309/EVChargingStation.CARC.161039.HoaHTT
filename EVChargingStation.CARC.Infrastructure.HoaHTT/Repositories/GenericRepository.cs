...
namespace EVChargingStation.CARC.Infrastructure.HoaHTT.Repositories
{
    public class GenericRepository<TEntity> : IGenericRepository<TEntity> where TEntity : BaseEntity
    {
        ...
        public GenericRepository(FA25_SWD392_SE161039_G6_EvChargingStation context, ICurrentTime timeService, IClaimsService claimsService)
        {
        ...
        public async Task<TEntity?> GetByIdAsync(Guid id, params Expression<Func<TEntity, object>>[] includes)
        {
            IQueryable<TEntity> query = _dbSet;
            foreach (var include in includes) query = query.Include(include);
            var result = await query.FirstOrDefaultAsync(x => x.HoaHTTID == id);
            return result;
        }
        ...
        {
            var entities = await _dbSet.Where(e => entitiesId.Contains(e.HoaHTTID)).ToListAsync();
