...
namespace EVChargingStation.CARC.WebAPI.HoaHTT.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        [ProducesResponseType(typeof(ApiResult<LoginResponceDto>), 200)]
        [ProducesResponseType(typeof(ApiResult<LoginResponceDto>), 400)]
        public async Task<IActionResult> Login([FromBody] LoginRequestDto dto)
        {
            try
            {
                var result = await _authService.LoginAsync(dto, _configuration);
                return Ok(ApiResult<LoginResponceDto>.Success(result!, "200", "Login successful."));
            }
            catch (Exception ex)
            {
                var statusCode = ExceptionUtils.ExtractStatusCode(ex);
                var errorResponse = ExceptionUtils.CreateErrorResponse<LoginResponceDto>(ex);
